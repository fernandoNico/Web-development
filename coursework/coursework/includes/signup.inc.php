...
if(isset($_POST['captcha_txt']))  {
	...
	if($_SESSION['captcha_text']==md5(strtolower($_POST['captcha_txt']))){
		...
		// Insert data to DB
		$sql = "INSERT INTO users (username, email, password, active_user, authentication_code ) 
		values ('$username','$email','$hashpwd ','$captchaInitialValue','$code');";
		...
		header("Location: ../verifyAccount.php?signup=success");
		exit();
	}
	else{
